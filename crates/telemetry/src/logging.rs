...
const LOG_FILE_NAME_PREFIX: &str = "palmtop.log";
...
pub fn build_subscriber(verbose: bool, appender: Option<RollingFileAppender>) -> Vec<WorkerGuard> {
    ...
    let stdout_env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new(match verbose {
            true => "palmtop=debug,network=debug".to_owned(),
            false => "palmtop=info,network=debug".to_owned(),
        })
    });
    ...
    if let Some(appender) = appender {
        ...
        let file_env_filter = EnvFilter::from("palmtop=debug,network=debug");
